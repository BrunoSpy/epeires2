...
;(function ($) {
	$.widget('epeires.timeline', {
		...
		_create: function () {
			var self = this
			var height =
				$(window).height() - this.options.topOffset + 'px'
			...
			if (this.options.view == 'day') {
				...
				if (urlEvent.indexOf('?') > 0) {
					urlEvent +=
						'&day=' + self.currentDay.toUTCString()
				} else {
					urlEvent +=
						'?day=' + self.currentDay.toUTCString()
				}
			...
			$.when(
				...
				if (self.options.view == 'day') {
					...
					if (!self._isToday(self.currentDay)) {
						self.element
							.find('#timeline-background')
							.addClass('anotherday')
					} else {
						self.element
							.find('#timeline-background')
							.removeClass('anotherday')
					}
						...
						function (data, textStatus, jqHXR) {
							if (jqHXR.status !== 304) {
								...
								self.lastupdate = new Date(
									jqHXR.getResponseHeader('Last-Modified')
								)
							}
						}
					)
			...
			$(window).resize(function () {
				var height =
					$(window).height() - self.options.topOffset + 'px'
...
*/
					...
					mouseenter: function () {
						...
						$(this)
							.tooltip({
								...
								viewport: '#timeline',
								template:
									'<div class="tooltip tooltip-actions" role="tooltip"><div class="tooltip-arrow"></div><div class="tooltip-inner"></div></div>',
							})
							.tooltip('show')
				...
				$.post(
					...
					function (data) {
						...
						if (data['event']) {
							...
							var sendToMattermost =
								event.mattermostid !== null
			...
			this.element.on(
				...
				function (e) {
					if (
						$(e.target).hasClass('rect_elmt') ||
						$(e.target).hasClass('elmt_name')
					) {
			...
			this.element.on(
				...
				function (e) {
					...
					var id = me.data('id')
					var txt =
						'<p class="elmt_tooltip actions">' +
						'<p><a href="#" data-id="' +
						id +
						'" class="send-evt"><span class="glyphicon glyphicon-envelope"></span> Envoyer ' +
						i18n.t('ipo.IPO') +
						'</a></p>'
					var event = self.events[self.eventsPosition[id]]
					if (self.options.mattermost) {
						txt +=
							...
							event.mattermostid +
							'" ' +
							'class="send-mattermost"><span class="glyphicon glyphicon-send"></span> ' +
							(event.mattermostid == null
								? 'Envoyer sur le chat'
								: 'Mettre à jour sur le chat') +
							'</a>'
					}
					if (event.status_id < 4) {
						//non annulé et non supprimé
						...
						if (event.punctual === false) {
							if (event.star === true) {
								txt +=
									'<p><a href="#" data-id="' +
									id +
									'" class="evt-non-important"><span class="glyphicon glyphicon-leaf"></span> Non important</a></p>'
							} else {
								txt +=
									'<p><a href="#" data-id="' +
									id +
									'" class="evt-important"><span class="glyphicon glyphicon-fire"></span> Important</a></p>'
							}
						}
						txt +=
							'<p><a href="#add-note-modal" class="add-note" data-toggle="modal" data-id="' +
							id +
							'"><span class="glyphicon glyphicon-comment"></span> Ajouter une note</a></p>'
						if (event.modifiable && !event.readonly) {
							txt +=
								'<p><a href="#" data-id="' +
								id +
								'" class="cancel-evt"><span class="glyphicon glyphicon-remove"></span> Annuler évènement</a></p>'
						}
					}
					if (
						event.status_id < 5 &&
						...
						txt +=
							'<p><a href="#" data-id="' +
							id +
							'" class="delete-evt"><span class="glyphicon glyphicon-trash"></span> Supprimer</a></p>'
					}
					txt += '</p>'
					me.popover({
						container: '#timeline',
						...
						viewport: '#timeline',
						template:
							'<div class="popover label_elmt" role="tooltip"><div class="arrow"></div><h3 class="popover-title"></h3><div class="popover-content"></div></div>',
					}).popover('show')
		...
		addEvent: function (event, sort) {
			...
			if (
				event.id in this.eventsPosition &&
				this.events[this.eventsPosition[event.id]]
			) {
		...
		addEvents: function (eventsList, sendToMattermost) {
			...
			$.each(eventsList, function (key, value) {
				...
				if (
					typeof sendToMattermost !== 'undefined' &&
					sendToMattermost == true
				) {
		...
		day: function (date) {
			...
			if (this.dayview) {
				...
				if (this._isValidDate(tempday)) {
					if (
						self._isValidDate(self.currentDay) &&
						self._isSameDay(self.currentDay, tempday)
					) {
				...
				if (refresh) {
					$.when(self._refreshCategories()).then(
						function () {
							//we need updated categories before getting events
							$.when(
								$.getJSON(
									url,
									function (data, textStatus, jqHXR) {
			...
			if (
				...
				if (this.options.showCategories === true) {
					...
					this.events.sort(function (a, b) {
						if (self.options.showOnlyRootCategories) {
							var aPosition =
								self.catPositions[a.category_root_id] ===
								undefined
									? -1
									: self.catPositions[a.category_root_id]
							var bPosition =
								self.catPositions[b.category_root_id] ===
								undefined
									? -1
									: self.catPositions[b.category_root_id]
							...
							if (a.category_place < b.category_place) {
								...
								a.category_place > b.category_place
							) {
								return 1
							}
						} else {
							var aPosition =
								self.catPositions[a.category_id]
							var bPosition =
								self.catPositions[b.category_id]
							if (aPosition < bPosition) {
								return -1
							} else if (aPosition > bPosition) {
								return 1
		...
		sortCategories: function (comparator) {
			if (
				...
				this.lastCategoriesComparator === undefined
			) {
		...
		filter: function (callback) {
			...
			if (
				cb === 'default' ||
				(cb === undefined && this.lastFilter === undefined)
			) {
				cb = function (event) {
		...
		_updateView: function (full) {
			...
			if (this.dayview) {
				...
				if (
					this._isToday(this.currentDay) &&
					this.center24
				) {
					//if today -> center the timeline around actual hour
					var now = new Date()
					this.timelineBegin = new Date(
						now.getFullYear(),
						now.getMonth(),
						now.getDate(),
						now.getHours() - 11,
					...
					this.timelineEnd = new Date(
						...
						now.getMonth(),
						now.getDate(),
						now.getHours() - 11 + this.timelineDuration,
						0,
						0
					)
				} else {
					this.currentDay = new Date(
						Date.UTC(
							this.currentDay.getUTCFullYear(),
							this.currentDay.getUTCMonth(),
							this.currentDay.getUTCDate()
						)
					)
					this.timelineBegin = new Date(
						Date.UTC(
					...
					this.timelineEnd = new Date(
						Date.UTC(
							this.timelineBegin.getFullYear(),
							this.timelineBegin.getMonth(),
							this.timelineBegin.getDate(),
				...
				this.timelineBegin = new Date(
					...
					now.getMonth(),
					now.getDate(),
					now.getHours() - this.options.pasthours,
					0,
					0
				)
				this.timelineEnd = new Date(
			...
			var maxY =
				self._updateYPosition(true) +
				self.options.eventHeight
		...
		_computeX: function (date) {
			if (
				date < this.timelineBegin ||
				date > this.timelineEnd
			) {
				return -1
			} else {
				return (
					((date - this.timelineBegin) / (1000 * 60 * 60)) *
					this.intervalle *
					2
				)
			}
		...
		_computeY: function (event) {
			...
			if (this.options.showCategories) {
				var catPos = this.options.showOnlyRootCategories
					? this._getCategoryPosition(
							event.category_root_id
					  )
				...
				if (i === 0) {
					...
					var top =
						this.params.topSpace + this.params.catSpace
					for (var j = 0; j < catPos; j++) {
						top +=
							this._getCategoryMinHeight(
								this.categories[j]
							) + this.params.catSpace
					}
					return top
				} else {
					...
					if (
						(this.options.showOnlyRootCategories &&
							prevEvent.category_root_id !==
								event.category_root_id) ||
						(!this.options.showOnlyRootCategories &&
							prevEvent.category_id !== event.category_id)
					) {
						//calcul du bas de la catégorie non vide précédente
						var prevCatPos = this.options
							.showOnlyRootCategories
							? this._getCategoryPosition(
									prevEvent.category_root_id
							  )
							: this._getCategoryPosition(
									prevEvent.category_id
							  )
						...
						for (var j = 0; j < catEvents.length; j++) {
							var line =
								this.eventsYPosition[catEvents[j].id]
							if (
								!isNaN(line) &&
							...
							for (var k = 0; k < eventsLine.length; k++) {
								...
								if (
									...
									if (cat.compact === false) {
										var sizeWithoutLabel =
											this._tryRemoveLabel(evt)
						...
						var allCatEvents = this.eventsDisplayed.slice(
							0,
							i
						)
						allCatEvents = this.options
							.showOnlyRootCategories
						...
						for (var j = 0; j < allCatEvents.length; j++) {
							var ypos =
								this.eventsYPosition[allCatEvents[j].id]
						...
						return (
							max +
							this.options.eventHeight +
							this.params.eventSpace
						)
					}
				}
			} else {
				if (this.options.compact) {
					if (i === 0) {
						...
						var lines = this.eventsYPosition.filter(
						...
						for (var index in lines) {
							...
							for (var id in this.eventsYPosition) {
								if (
									this.eventsYPosition[id] === lines[index]
								) {
									eventsLine.push(id)
								}
							}
						...
						return (
							...
							Math.max.apply(
								null,
								this.eventsYPosition.filter(function (val) {
									return !isNaN(val)
								})
							)
						)
		...
		_drawBase: function () {
			...
			var time_obj = $(
				'<div class="Time_obj horiz_bar"></div>'
			)
			var base_elmt = $(
				'<div id="timeline-base" class="Base"></div>'
			)
			base_elmt.css({
				'margin-left': this.options.leftOffset + 'px',
				width:
			...
			var tailleTotaleHoursAndHalves =
				nbHours * 37 + (nbHours - 1) * 20 //20px = taille de la boite des demi-heures
			//si taille des heures + demi >= largeur dispo, on ne dessine plus les demi
			var drawHalves =
				tailleTotaleHoursAndHalves < largeurDisponible
			//on supprime l'affichage de certaines heures jusqu'à ce que ça tienne
			var modulo = 1
			while (
				tailleTotaleHours / modulo >
				largeurDisponible
			) {
				modulo++
			}
			...
			for (
				var i = 0;
				i < this.timelineDuration * 2 + 1;
				i++
			) {
				var vert_bar = $(
					'<div class="Time_obj vert_bar"></div>'
				)
				...
				vert_bar.css({
					...
					width: 1,
					height:
						'calc(100% - ' + this.params.topSpace + 'px)',
					'background-color': '#C0C0C0',
				})
				if (i % 2 === 1) {
					if (drawHalves) {
						var halfHour = $(
							'<div class="Time_obj halfhour">30</div>'
						)
					...
					if ((i / 2) % modulo === 0) {
						var roundHour = $(
							'<div class="Time_obj roundhour">' +
								this._formatNumberLength(h_temp, 2) +
								':00</div>'
						)
		...
		_drawTimeBar: function () {
			if ($('#TimeBar').length > 0) {
				...
				TimeBar.css({
					top:
						/*this.options.topOffset + */ this.params
							.topSpace + 'px',
					height:
						'calc(100% - ' +
						(this.params.topSpace + 5) +
						'px)',
				})
				if (x > 0) {
					TimeBar.css('left', x + '%')
		...
		_updateTimebar: function () {
			...
			var diff =
				(now - this.timelineBegin) / (1000 * 60 * 60) //différence en heure
			...
			if (this.dayview === true) {
				...
				var currentDayUTC = Date.UTC(
					this.currentDay.getUTCFullYear(),
					this.currentDay.getUTCMonth(),
					this.currentDay.getUTCDate()
				)
				if (this.lastUpdateTimebar !== undefined) {
					var lastUpUTC = Date.UTC(
						this.lastUpdateTimebar.getUTCFullYear(),
						this.lastUpdateTimebar.getUTCMonth(),
						this.lastUpdateTimebar.getUTCDate()
					)
				}
				if (
					lastUpUTC !== undefined &&
					...
					Math.ceil(
						(nowUTC - lastUpUTC) / (1000 * 60 * 60 * 24)
					) !== 0 &&
					//si le jour affiché est la veille
					Math.ceil(
						(nowUTC - currentDayUTC) / (1000 * 60 * 60 * 24)
					) === 1
				) {
					this.day(now)
					...
					if ($('#date').length > 0) {
						...
						var daystring =
							FormatNumberLength(day, 2) +
							'/' +
							FormatNumberLength(month, 2) +
							'/' +
							FormatNumberLength(year, 4)
						$('#date').val(daystring)
					}
			...
			for (let i = 0; i < this.categories.length; i++) {
				...
				if (this._isCategoryEmpty(curCat)) {
					...
					if (curCat.short_name.length > 4) {
						shorttext =
							curCat.short_name.substring(0, 4) +
							'<span>⋮</span>'
					} else {
						shorttext = curCat.short_name
					}
					text_cat =
						'<div class="verticaltxt reduced">' +
						shorttext +
						'</div>'
				} else {
					text_cat =
				...
				if ($('#category' + curCat.id).length === 0) {
					...
					$('#category').append(cat)
					cat.css({
						'background-color': curCat.color,
						height: 'auto',
						left: '15px',
					})
					if (
						this._yiq(this._hex2rgb(curCat.color)) < 0.5
					) {
						cat.css('color', '#000')
					} else {
						cat.css('color', '#fff')
				...
				$.getJSON(
					...
					function (data) {
						...
						$.each(res, function (index, value) {
							txt +=
								'<p><a class="btn btn-sm use-model" data-id="' +
								value.key +
								'" data-name="' +
								value.name +
								'" title="' +
								...
								value.name +
								'</span> <span class="pull-right glyphicon glyphicon-share-alt"></span></a></p>'
						})
						$('#category' + catid).popover({
							...
							template:
								'<div class="popover quickaccess" role="tooltip"><div class="arrow"></div><h3 class="popover-title"></h3><div class="popover-content"></div></div>',
						})
						$('#category' + catid).on(
							'mouseenter',
							function (e) {
								$(
									'.category:not("#category' + catid + '")'
								).popover('hide')
								$(this).popover('show')
							}
						)
		...
		_isCategoryEmpty: function (category) {
			if (this.options.showOnlyRootCategories) {
				return (
					this.eventsDisplayed.filter(
						(event) =>
							event.category_root_id === category.id
					).length === 0
				)
			} else {
				return (
		...
		_drawEvent: function (event) {
			...
			if (event.display && cat !== null) {
				...
				if (
					this.element.find('#event' + event.id).length ===
					0
				) {
					...
					this._doDrawEvent(
						event,
						this.element.find('#event' + event.id),
						cat
					)
				}
			} else {
		...
		_doDrawEvent: function (event, elmt, categ) {
			...
			var textColor =
				self._yiq(self._hex2rgb(couleur)) >= 0.5
					? '#fff'
					: '#000'
			...
			if (event.recurr == true) {
				name +=
					' <span data-toggle="tooltip" data-container="body" data-placement="bottom" data-title="' +
					event.recurr_readable +
					'" class="badge recurrence">R</span>'
			} else {
				elmt.find('.modify-evt').data('recurr', '')
			}
			if (event.scheduled > 0) {
				name +=
					' <a href="#"><span class="badge scheduled">P</span></a>'
			}
			if (this.options.fileIcon && event.files > 0) {
				name +=
					' <a href="#"><span class="badge files"><span class="glyphicon glyphicon-file"></span></span></a>'
			}
			...
			var hDeb =
				this._formatNumberLength(
					startdate.getUTCHours(),
					2
				) +
				':' +
				this._formatNumberLength(startdate.getMinutes(), 2)
			...
			if (enddate !== -1) {
				var hEnd =
					...
					':' +
					this._formatNumberLength(enddate.getMinutes(), 2)
			...
			var txtSize = this._computeTextSize(
				elmt_txt.text().trim(),
				elmt_txt.css('font-style'),
				'RobotoDraft',
				'700',
				'14px'
			)
			...
			if (event.punctual) {
				...
				var place =
					((100 - x_deb) * largeurDisponible) / 100

				if (larg + debWidth + txt_wid < place) {
					// s'il reste assez de place à droite du rectangle, on écrit le txt à droite
					elmt_txt.css({
				...
				elmt_txt
					.find('a > span.glyphicon')
					.css('color', textColor)

				/* 4: positionnement final de la boite englobante */
				elmt.css({
					left: 'calc(' + x_deb + '% - ' + offset + 'px)',
					width: totalWidth + 'px',
				})
				...
				$.each(event.milestones, function (index, item) {
					...
					if (xMilestone > x_deb && xMilestone < x_end) {
						...
						var left =
							((xMilestone - x_deb) / (x_end - x_deb)) * 100
						milestone.css({
							left: left + '%',
							color: textColor,
						})
				...
				var rectPixels =
					((x_end - x_deb) * largeurDisponible) / 100
				if (rectPixels < 40) {
				...
				if (txt_wid + 40 < rectPixels) {
					...
					var place =
						((100 - x_end) * largeurDisponible) / 100
					lien.addClass('disp').show()
					elmt_txt.addClass('outside')
					textColor =
						event.status_id == 4 ? 'grey' : 'black'
					elmt_txt.css('color', textColor)
					elmt_txt
						.find('a > span.glyphicon')
						.css('color', textColor)
					if (endWidth + txt_wid < place) {
						// s'il reste assez de place à droite du rectangle, on écrit le txt à droite
						elmt_txt.css({
							left: 'calc(100% - ' + txt_wid + 'px)',
						})
						totalWidth += txt_wid
						lien.css({
							left: -endWidth + 'px',
						...
						offset += txt_wid
						lien.css({
							left: txt_wid - txtOffset + 'px',
							width: debWidth + txtOffset + 'px',
						})
			...
			event.xleft =
				(x_deb * largeurDisponible) / 100 - offset
			event.xright =
				(x_end * largeurDisponible) / 100 +
				totalWidth -
				offset
		...
		_removeLabel: function (event) {
			...
			if (elmt_txt.length > 0 && event.label === true) {
				...
				elmt
					.find('.lien')
					.removeClass('disp leftlink rightlink')
					.hide()
				//on cache le txt
				elmt_txt
					.find('span.elmt_name')
		...
		_updateStatus: function (event, elmt, textColor) {
			...
			switch (event.status_id) {
				case 1: //nouveau
					...
					if (now > start) {
						...
						if (event.modifiable && !event.readonly) {
							elmt_deb.tooltip({
								title:
									"Cliquer pour confirmer l'heure de début.",
								container: 'body',
							})
						} else {
					...
					if (event.punctual || event.end_date === null) {
						...
						if (this._isValidDate(end) && now > end) {
							...
							if (event.modifiable && !event.readonly) {
								elmt_fin.tooltip({
									title:
										"Cliquer pour confirmer l'heure de fin.",
									container: 'body',
								})
							} else {
				...
				case 4: //annulé
					...
					elmt_txt.css({
						'font-style': 'normal',
						color: this._shadeHexColor(newcolor, -0.5),
					})
					rect.removeClass('stripes')
		...
		_getSkeleton: function (event) {
			...
			var elmt_txt = $(
				'<p class="label_elmt"><span class="elmt_name">' +
					event.name +
					'</span></p>'
			)
			...
			var elmt_b1 = $(
				'<a href="#" class="modify-evt" data-id="' +
					event.id +
					'" data-name="' +
					event.name +
					'" data-recurr="' +
			...
			if (this.options.opentab === true) {
				var elmt_b2 = $(
					'<a href="#" class="checklist-evt" data-id="' +
						event.id +
						'" data-name="' +
						event.name +
						'"></a>'
			...
			var elmt_b3 = $(
				'<a href="#" class="tooltip-evt" data-id="' +
					event.id +
					'"></a>'
			)
			elmt_b3.append(
			...
			var elmt_deb = $(
				'<a href="#" class="elmt_deb"><span class="glyphicon"></span><span class="hour-txt"></span></a>'
			)
			elmt_rect.append(elmt_deb)
			var elmt_fin = $(
				'<a href="#" class="elmt_fin"><span class="hour-txt"></span><span class="glyphicon"></span></a>'
			)
		...
		_sendToMattermost: function (event) {
			if (this.options.mattermost) {
				...
				var message =
					...
					'\n' +
					'* _Début_ : ' +
					moment(event.start_date).utc().format('lll') +
					'\n'
				if (event.end_date) {
					message +=
						'* _Fin_ : ' +
						moment(event.end_date).utc().format('lll') +
						'\n'
				}
				message += '### Description' + '\n'
				$.each(event.fields, function (nom, contenu) {
		...
		_isEventInTimeline: function (event) {
			...
			if (
				(event.punctual &&
					startdate >= this.timelineBegin &&
					startdate <= this.timelineEnd) ||
				(!event.punctual &&
					startdate < this.timelineEnd &&
					enddate === null) ||
				(!event.punctual &&
					enddate !== null &&
					enddate > this.timelineBegin &&
					startdate < this.timelineEnd)
			) {
				return true
			} else {
		...
		_computeTextSize: function (
			...
			jQuery._cacheCanvas.getContext('2d').font =
				fontStyle +
				' ' +
				fontWeight +
				' ' +
		...
		_beforeEventMove: function (
			...
			var pix_time =
				(30 * 60000) /
				((self.intervalle * self._getWidthAvalaible()) /
					100)
			var elmt_fin = elmt.find('.elmt_fin')

			var id = elmt.data('ident')
			var event =
				self.eventsDisplayed[
					self.eventsDisplayedPosition[id]
				]
			...
			if (moveEvent == 'touchmove') {
				...
				if (
					clientX <
					rect_elmt.offset().left + rect_width / 2
				) {
			...
			self.element.on(moveEvent, function (e2) {
				...
				switch (moveEvent) {
					...
					case 'touchmove':
						lastClientX =
							e2.originalEvent.changedTouches[0].clientX
				}
				...
				switch (startOrEnd) {
					case 1:
						if (delt2 + 40 < rect_width) {
							temp_deb.setTime(
								start.getTime() + delt2 * pix_time
							)
							var txtHour =
								self._formatNumberLength(
									temp_deb.getUTCHours(),
									2
								) +
								':' +
								self._formatNumberLength(
							...
							if (
								elmt.find('.lien').hasClass('rightlink')
							) {
					...
					case 2:
						if (rect_width + delt2 > 40) {
							...
							temp_fin.setTime(
								d_fin.getTime() + delt2 * pix_time
							)
							var txtHour =
								self._formatNumberLength(
									temp_fin.getUTCHours(),
									2
								) +
								':' +
								self._formatNumberLength(
