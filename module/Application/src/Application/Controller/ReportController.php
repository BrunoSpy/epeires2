...
class ReportController extends AbstractActionController {
    ...
    public function dailyAction(){       
        ...
        if($day){
            ...
            foreach ($cats as $cat) {
                ...
                $category['events'] = $objectManager->getRepository('Application\Entity\Event')
                                                    ->getEvents($this->zfcUserAuthentication(), $day, null, true, array($cat->getId()));
                $category['childs'] = array();
                foreach ($cat->getChildren() as $subcat) {
                    $subcategory = array();
                    $subcategory['events'] = $objectManager->getRepository('Application\Entity\Event')
    ...
    public function reportAction(){
        ...
        foreach ($cats as $cat) {
            ...
            $category['name'] = $cat->getName();
            $category['events'] = $objectManager->getRepository('Application\Entity\Event')
                                            ->getEvents(null, $day, null, true, array($cat->getId()));
            $category['childs'] = array();
            foreach ($cat->getChildren() as $subcat) {
                $subcategory = array();
                $subcategory['events'] = $objectManager->getRepository('Application\Entity\Event')
                                        ->getEvents(null, $day, null, true, array($subcat->getId()));
                $subcategory['name'] = $subcat->getName();
                $category['childs'][] = $subcategory;
            }
            $eventsByCats[] = $category;
