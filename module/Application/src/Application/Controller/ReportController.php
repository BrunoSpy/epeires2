...
class ReportController extends AbstractActionController
{
        ...
        if ($day) {
            ...
            foreach ($cats as $cat) {
                ...
                $category['events'] = $objectManager
                    ...
                    ->getEvents($this->zfcUserAuthentication(), $day, null, true, array($cat->getId()), array(1,2,3,4));
                $category['childs'] = array();
                foreach ($cat->getChildren() as $subcat) {
                    $subcategory = array();
                    $subcategory['events'] = $objectManager
                        ->getRepository('Application\Entity\Event')
                        ->getEvents($this->zfcUserAuthentication(), $day, null, true, array($subcat->getId()), array(1,2,3,4));
    ...
    public function reportAction()
    {
        ...
        foreach ($cats as $cat) {
            ...
            $category['events'] = $objectManager
                ->getRepository('Application\Entity\Event')
                ->getEvents(null, $day, null, true, array($cat->getId()), array(1,2,3,4));
            $category['childs'] = array();
            foreach ($cat->getChildren() as $subcat) {
                $subcategory = array();
                $subcategory['events'] = $objectManager
                    ->getRepository('Application\Entity\Event')
                    ->getEvents(null, $day, null, true, array($subcat->getId()), array(1,2,3,4));
                $subcategory['name'] = $subcat->getName();
                $category['childs'][] = $subcategory;
            }
            $eventsByCats[] = $category;
